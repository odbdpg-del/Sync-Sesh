...
export function useSoundEffects(state: DabSyncState, lobbyState: DerivedLobbyState, countdownDisplay: CountdownDisplayState) {
  ...
  const previousUrgentSecondRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    ...
    if (state.session.phase === "countdown") {
      const urgentSecond = countdownDisplay.timerText;

      if (
        countdownDisplay.isUrgent &&
        urgentSecond !== previousUrgentSecondRef.current &&
        urgentSecond !== "00:00"
      ) {
        playCue("count_urgent_tick");
      }

      previousUrgentSecondRef.current = urgentSecond;
    } else {
      previousUrgentSecondRef.current = null;
    }
