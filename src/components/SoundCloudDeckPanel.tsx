...
interface SoundCloudDeckPanelProps {
...
function SoundCloudDeckCard({ deck, side, grid }: { deck: SoundCloudDeckPanelConfig; side: SoundCloudDeckSide; grid: SoundCloudGridController }) {
  ...
  const isTrackLoaded = Boolean(state.currentTrackUrl) && state.playbackDuration > 0 && state.isWidgetReady;
  const deckStateLabel = state.isPlaying ? "Playing" : isTrackLoaded ? "Paused" : "Standby";
  const trackTitle = isTrackLoaded ? state.currentTrackTitle : "Ready for a playlist";
  const trackArtist = isTrackLoaded ? state.currentTrackArtist : "Choose Play or Shuffle to load a track.";
  const showTrackLinks = Boolean(state.currentTrackUrl || state.currentTrackArtistUrl);
  const stateNote = state.errorMessage ?? (isTrackLoaded
    ? "Advanced controls are live for this deck."
    : "Load a track to unlock cues, seek, and burst-grid timing tools.");

  return (
    <article className={`soundcloud-deck soundcloud-deck-${deck.id.toLowerCase()}`}>
      <div className={`soundcloud-player soundcloud-deck-card ${state.errorMessage ? "soundcloud-error-state" : ""} ${isTrackLoaded ? "" : "soundcloud-deck-card-empty"}`}>
        ...
        <div className="soundcloud-player-main soundcloud-deck-main">
          <div className="soundcloud-deck-overview">
            <div className="soundcloud-artwork soundcloud-deck-artwork" aria-hidden="true">
              {state.artworkUrl ? <img src={state.artworkUrl} alt="" /> : <span>{deck.id}</span>}
            </div>

            <div className="soundcloud-track-row soundcloud-deck-track-row">
          ...
          <div className="soundcloud-deck-core">
            ...
            <div className="soundcloud-controls soundcloud-deck-transport">
              <div className="soundcloud-actions soundcloud-deck-transport-actions">
              ...
              <div className="soundcloud-volume-row soundcloud-deck-trim-row">
          ...
          <div className="soundcloud-deck-utility">
            ...
            <p className="soundcloud-player-note soundcloud-deck-state-note">{stateNote}</p>
          </div>

          <div className={`soundcloud-deck-advanced ${isCueBlocked ? "soundcloud-deck-advanced-blocked" : ""}`}>
            <div className="soundcloud-deck-advanced-header">
              <p className="meta-label">Advanced deck tools</p>
              <span>{isCueBlocked ? "Load a track to arm tools" : "Cue, seek, and burst controls ready"}</span>
            </div>

            <div className="soundcloud-deck-advanced-grid">
              <SoundCloudCueStrip
              ...
              <div className="soundcloud-deck-grid-shell">
          ...
          {showTrackLinks ? (
            <div className="soundcloud-track-actions soundcloud-deck-links-compact" aria-label={`${deck.label} quick links`}>
              {state.currentTrackUrl ? (
                <a className="soundcloud-open-link" href={state.currentTrackUrl} target="_blank" rel="noreferrer">
                  Open
                </a>
...
export function SoundCloudDeckPanel({
  decks,
  gridControllers,
  mixer,
  mixerReadout,
  ...
  return (
    <section className="panel soundcloud-panel" hidden={!isVisible}>
